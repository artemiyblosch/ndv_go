...
func NewWorld(
	...
	return World{projections, bg, domain, img}
}
...
func (world World) Project(point Point) Point {
	result := make(Point, len(point))
	copy(result, point)
...
func (world World) DomainToPixelCoords(point Point) [2]float64 {
	return [2]float64{
...
func (world World) DrawFace(points [][]float64, faceColor color.NRGBA) {
	...
	gc.SetLineWidth(1)
	gc.MoveTo(p_coords[0][0], p_coords[0][1])
